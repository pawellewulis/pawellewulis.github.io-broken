...
This problem is an excellent example of how to decompose a larger task into a collection of smaller, more manageable ones.
...
Let us ponder for a moment the dynamics behind the solution and formalize the discussion a bit. It is natural to start by analyzing some obvious questions. A tempting first try:
...
For instance, in one of the examples above we had $x = \text{"2+2=4"}$. In such cases, we expect one guardian to answer Yes and the other one to answer No. However, we also saw that the question "Do you have the key?" can result in two No's (or two Yes's). This happens because we are effectively asking two different questions, depending on the recipient. To align with the formalization, we may label the guardians as Guardian A and Guardian B, and consider a statement such as

$$ x = \text{"Guardian A has the key."}$$

In practice, however, we will sometimes use simpler formulations such as "You have the key."

Now the logical value of the statement remains independent of the recipient.

Let us refer to the truth-telling guardian as $T$ and the lying one as $F$. The solution to the two-guardians puzzle revolves around the idea of forcing both guardians to behave like liars. If $T$ is asked to answer as $F$ would, the response must be false. If $F$ is asked to answer as $T$ would, he still lies, since he cannot do otherwise.
...
$$
  11 \mapsto \text{Barrel 4 or Barrel 5 is poisoned.}
$$
...
Let us label the aliens as Alien A and Alien B, not in the most creative way. Notice that we cannot determine the logical values of the following two statements at the same time:
...
Let us first observe that if $R$ also always spoke truthfully, the puzzle would become easy. 

We could simply split the $6$ possible assignments into two groups of three, say Group A and Group B, and ask:

$$\text{Is the correct assignment in this Group A?}$$
...
We are now done, because two questions are exactly enough to identify which of the $4$ candidate permutations is correct!
...
Let us take a step back. Any question we ask is, in effect, meant to identify a subset of permutations of $ T,F,R $ containing the true arrangement. In this sense, it can be viewed as asking something of the form:
...
Let us briefly explain why the second condition is necessary. Suppose our question concerns the subset

$$ \mathcal{S} = \{ (R,F,T) , (F,R,T), (F,T,R)\}. $$

The diagram below shows which permutations remain possible depending on whether the goddess lies or not. Remember that if $G_1$ lies, then necessarily $G_1 = R$. 
...
If $G_2 \not= R$, then the answer can at best split the remaining $3$ permutations into two groups and eliminate one of them. However, if $G_2=R$, the permutation $(F,R,T)$ remains possible. Since we do not know which of these cases holds, at least $3$ permutations must still be considered. This is too many to resolve with a single remaining question. 

The same argument applies if we direct the second question to $G_1$ or $G_3$ instead.

#### Counting the Possible Solutions

We now understand the conditions that the first question must satisfy. Let us count the valid choices of $\mathcal{S}$, the subset of permutations encoded within the question. First, whether $\mathcal{S}$ contains either $(R,F,T)$ or $(R,T,F)$ doesn't matter, since both remain possible in any case; at this stage, we cannot exclude $G_1=R$. Among the remaining $4$ permutations, we must distinguish between the cases $G_2=R$ and $G_3=R$, so that we can identify a non-$R$ goddess and direct the remaining two questions to her.
...
- Must contain either the $2$ permutations corresponding $G_2=R$ or the $2$ permutations corresponding to $G_3=R$, but not all four.

This gives $8$ valid first questions in total, so $1/8=12.5 \%$ of all possible choices lead to a complete solution.
